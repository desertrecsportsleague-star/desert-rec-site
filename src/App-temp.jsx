...
if (paidUnassigned.length < 9) {
alert("You need at least 9 paid unassigned free agents to form a free agent team.");
return;
}

const females = paidUnassigned.filter((p) => (p.gender || "").toLowerCase() === "female");
const others = paidUnassigned.filter((p) => (p.gender || "").toLowerCase() !== "female");

if (females.length < 3) {
alert("You need at least 3 paid unassigned female free agents to form a free agent team.");
return;
}
...
(player) => player.assignedTeamId === freeAgentTeam.id
).length;

const roomLeft = Math.max(12 - currentlyAssignedCount, 0);

if (roomLeft === 0) {
alert(`${FREE_AGENT_TEAM_NAME} is already full at 12 players.`);
return;
}

const selectedFemales = females.slice(0, Math.min(3, roomLeft));
...
alert("No free agents were assigned.");
return;
}
...
await loadData();
alert(
`${selectedPlayers.length} paid free agents were assigned to ${FREE_AGENT_TEAM_NAME}. A captain was selected automatically.`
);
} catch (err) {
alert(err.message || String(err));
} finally {
...
if (targetTeamId === undefined) throw new Error("Select a team.");
...
Auto-creates or fills the free agent team only when there are at least 9 paid unassigned players and at least 3 paid unassigned females. Assigns up to 12 max and selects a captain automatically.
</div>
</div>
