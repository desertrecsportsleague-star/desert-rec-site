...
export default function App() {
  ...
  async function handleAdminLogin(e) { e.preventDefault(); try { if (!loginEmail.trim()) throw new Error("Enter admin email."); if (!loginPassword.trim()) throw new Error("Enter password."); const { data, error } = await supabase.auth.signInWithPassword({ email: loginEmail.trim(), password: loginPassword }); if (error) throw new Error(error.message); if (data.user.email !== ADMIN_EMAIL) { await supabase.auth.signOut(); throw new Error("This account is not authorized for admin access."); } setUser(data.user); setLoginEmail(""); setLoginPassword(""); alert("Admin login successful."); } catch (err) { alert(err.message || String(err)); } }  async function handleLogout() { await supabase.auth.signOut(); }
  ...
  async function saveManualStanding(team) {
    try {
      ...
      const runsAgainst = Number(getStandingInput(team.teamId, "runsAgainst", team.runsAgainst)) || 0;
      const differentialValue = getStandingInput(team.teamId, "differential", runsFor - runsAgainst);
  ...
  return (
    <div>
      ...
      {page === "standings" && <div className="page"><h2 className="section-title">Standings</h2><div className="grid3"><div><h3>Current Softball</h3><StandingsTable list={softballStandings} /></div><div><h3>New Softball - July 16</h3><StandingsTable list={softballSummerStandings} /></div><div><h3>Kickball</h3><StandingsTable list={kickballStandings} /></div></div></div>}
