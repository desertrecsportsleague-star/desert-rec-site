...
function normalizeSeason(row, leagueType = "softball") {
if (row?.season_key) return row.season_key;
if (leagueType === "kickball") return "kickball_summer_2026";
return "softball_spring_2026";
}
...
const [standingForm, setStandingForm] = useState({});const [savingStandingTeamId, setSavingStandingTeamId] = useState("");
...
Enter weekly game scores above and save each game to update standings automatically. You may also override any team below. Use â€œUse Autoâ€ to return that team to score-based standings.
</p>
...
<div className="card"><h3>Roster Players</h3><table><thead><tr><th>Team</th><th>Name</th><th>Email</th><th>Waiver</th></tr></thead><tbody>{teamPlayers.filter((p) => p.leagueType === adminDataLeague && p.seasonKey === adminSeasonKey).map((player) => <tr key={player.id}><td>{teamNameById(player.teamId)}</td><td>{player.playerName}</td><td>{player.playerEmail || "â€”"}</td><td><StatusBadge status={player.signedWaiver ? "Signed" : "Missing"} /></td></tr>)}</tbody></table></div></>}</div>}
</div>
);
}
