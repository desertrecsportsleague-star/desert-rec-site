...
const heroImage = "/softball-action-running.jpeg";
const logoImage = "/desert-rec-banner.jpeg";
const homeGalleryImages = [
  { src: "/softball-action-running.jpeg", alt: "Desert Rec softball game action at Sundance Park" },
  { src: "/softball-action-slide.jpeg", alt: "Desert Rec softball player sliding safely into home" },
  { src: "/softball-champions.jpeg", alt: "Desert Rec softball champions celebrating after the game" },
];
...
const LEAGUES = {
  softball: {
    key: "softball",
    dataLeague: "softball",
    seasonKey: "softball_spring_2026",
    name: "Current Softball",
    title: "Current Thursday Night Coed Softball",
  ...
  softballSummer: {
    ...
    name: "Summer Softball",
    title: "New Thursday Night Coed Softball - Starts July 16",
    night: "Thursday Nights",
    location: "Sundance Park Fields 1 & 2",
    city: "Buckeye, Arizona",
    teamCap: 8,
    rosterTarget: "7 Men / 3 Women",
    freeAgentPrice: 60,
  ...
  kickball: {
    key: "kickball",
    dataLeague: "kickball",
    seasonKey: "kickball_summer_2026",
    name: "Kickball",
...
const SEASONS = {
  softball_spring_2026: {
    key: "softball_spring_2026",
    leagueType: "softball",
    label: "Softball Current Season",
    title: "Current Softball Season",
    startDate: "2026-05-14",
  },
  softball_summer_2026: {
    key: "softball_summer_2026",
    leagueType: "softball",
    label: "Softball Summer 2026",
  ...
  kickball_summer_2026: {
    ...
    title: "Kickball Summer Season",
    startDate: "2026-06-14",
  },
};

function seasonForLeague(leagueType) {
  if (leagueType === "kickball") return "kickball_summer_2026";
  return "softball_summer_2026";
}
...
function getLeagueDataKey(optionKey) { return LEAGUES[optionKey]?.dataLeague || optionKey; }
function getSeasonKey(optionKey) { return LEAGUES[optionKey]?.seasonKey || (optionKey === "kickball" ? "kickball_summer_2026" : "softball_spring_2026"); }
function normalizeSeason(row, fallbackLeague = "softball") {
  if (row?.season_key) return row.season_key;
  return normalizeLeague(row?.league_type || fallbackLeague) === "kickball" ? "kickball_summer_2026" : "softball_spring_2026";
}
...
function mapTeamRow(row) { return { id: row.id, teamName: row.team_name, captainName: row.captain_name, captainEmail: row.captain_email, captainPhone: row.captain_phone || "", paymentTier: row.payment_tier || "", paidStatus: row.paid_status || "Pending", inviteToken: row.invite_token || "", leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row), createdAt: row.created_at }; }
function mapFreeAgentRow(row) { return { id: row.id, playerName: row.player_name, playerEmail: row.player_email, playerPhone: row.player_phone || "", gender: row.gender || "", skill: row.skill || "", preferredPosition: row.preferred_position || "", paidStatus: row.paid_status || "Pending", assignedTeamId: row.assigned_team_id || null, leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row), createdAt: row.created_at }; }
function mapTeamPlayerRow(row) { return { id: row.id, teamId: row.team_id, playerName: row.player_name, playerEmail: row.player_email || "", playerPhone: row.player_phone || "", signedWaiver: !!row.signed_waiver, signatureName: row.signature_name || "", signedAt: row.signed_at || "", paidStatus: row.paid_status || "Pending", leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row) }; }
function mapScheduleRow(row) { return { id: row.id, week: row.week, date: row.game_date || "", time: row.game_time || "", field: row.field || "Sundance Park", status: row.status || "Scheduled", homeTeamId: row.home_team_id, awayTeamId: row.away_team_id, homeScore: row.home_score ?? "", awayScore: row.away_score ?? "", leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row) }; }
function mapManualStandingRow(row) {
  return {
    teamId: row.team_id,
    leagueType: normalizeLeague(row.league_type),
    seasonKey: row.season_key || normalizeSeason(row),
    seed: row.seed ?? "",
    wins: row.wins ?? 0,
    losses: row.losses ?? 0,
    ties: row.ties ?? 0,
    runsFor: row.runs_for ?? 0,
    runsAgainst: row.runs_against ?? 0,
    differential: row.differential ?? ((row.runs_for || 0) - (row.runs_against || 0)),
  };
}
function mapPlayerStatRow(row) { return { id: row.id, playerName: row.player_name || "", teamId: row.team_id || null, gamesPlayed: row.games_played ?? 0, hits: row.hits ?? 0, homeRuns: row.home_runs ?? 0, rbis: row.rbis ?? 0, runs: row.runs ?? 0, leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row) }; }
function mapPlayoffGameRow(row) { return { id: row.id, roundName: row.round_name || "", seedHome: row.seed_home ?? "", seedAway: row.seed_away ?? "", homeTeamId: row.home_team_id || null, awayTeamId: row.away_team_id || null, gameOrder: row.game_order ?? 1, homeScore: row.home_score ?? "", awayScore: row.away_score ?? "", winnerTeamId: row.winner_team_id || null, status: row.status || "Scheduled", leagueType: normalizeLeague(row.league_type), seasonKey: normalizeSeason(row) }; }
...
export default function App() {
  ...
  const [standingsLeague, setStandingsLeague] = useState("softballSummer");
  const [registrationLeague, setRegistrationLeague] = useState("softball");
  const [registrationSeason, setRegistrationSeason] = useState("softball_summer_2026");
  const [adminLeague, setAdminLeague] = useState("softball");
  const [adminSeason, setAdminSeason] = useState("softball_summer_2026");
  ...
  const [savingStandingTeamId, setSavingStandingTeamId] = useState("");
  ...
  const activeRegistrationSeason = SEASONS[registrationSeason] || SEASONS.softball_summer_2026;
  const activeAdminSeason = SEASONS[adminSeason] || SEASONS.softball_summer_2026;

  function recordMatchesSeason(record, seasonKey) {
    return (record.seasonKey || normalizeSeason(record, record.leagueType)) === seasonKey;
  }

  const registrationDataLeague = getLeagueDataKey(registrationLeague);
  const registrationSeasonKey = getSeasonKey(registrationLeague);
  const adminDataLeague = getLeagueDataKey(adminLeague);
  const adminSeasonKey = getSeasonKey(adminLeague);
  const registrationLeagueTeams = teams.filter((team) => team.leagueType === registrationDataLeague && team.seasonKey === registrationSeasonKey);
  const adminTeams = teams.filter((team) => team.leagueType === adminDataLeague && team.seasonKey === adminSeasonKey);
  const adminFreeAgents = freeAgents.filter((player) => player.leagueType === adminDataLeague && player.seasonKey === adminSeasonKey);
  const adminSchedule = schedule.filter((game) => game.leagueType === adminDataLeague && game.seasonKey === adminSeasonKey);
  const adminPlayerStats = playerStats.filter((stat) => stat.leagueType === adminDataLeague && stat.seasonKey === adminSeasonKey);
  const adminPlayoffGames = playoffGames.filter((game) => game.leagueType === adminDataLeague && game.seasonKey === adminSeasonKey);
  ...
  async function loadData() {
    try {
      const [{ data: teamRows }, { data: freeAgentRows }, { data: teamPlayerRows }, { data: scheduleRows }, { data: statRows }, { data: playoffRows }, { data: standingRows }] = await Promise.all([
        ...
        withTimeout(supabase.from("schedule_games").select("*").order("week", { ascending: true }), 15000),        optionalQuery(supabase.from("player_stats").select("*").order("player_name", { ascending: true })),
        optionalQuery(supabase.from("playoff_games").select("*").order("game_order", { ascending: true })),
        optionalQuery(supabase.from("team_standings").select("*")),
      ]);
  ...
  async function saveTeamRegistration() {
    ...
    const payload = { team_name: teamForm.teamName.trim(), captain_name: teamForm.captainName.trim(), captain_email: teamForm.captainEmail.trim(), captain_phone: teamForm.captainPhone.trim(), payment_tier: registrationDataLeague === "softball" ? pricing.tier : "kickball", paid_status: "Pending", signature_name: teamForm.signature.trim(), agree_waiver: teamForm.agreeWaiver, agree_captain_duty: teamForm.agreeCaptainDuty, agree_electronic: teamForm.agreeElectronic, invite_token: makeInviteToken(), league_type: registrationDataLeague, season_key: registrationSeasonKey };
  ...
  async function handleTeamCheckout(e) {
    ...
    try {
      ...
      alert(`${activeRegistrationLeague.name} team registration saved. Team payment will be handled separately by invoice.`);
    } catch (err) {
      alert(err.message || String(err));
    } finally {
  ...
  async function handleFreeAgentCheckout(e) {
    ...
    try {
      ...
      alert(`${activeRegistrationLeague.name} free agent registration saved. If payment is required, league staff will follow up separately.`);
    } catch (err) {
      alert(err.message || String(err));
    } finally {
  ...
  async function handleFindRosterTeam(e) {
    ...
    try {
      const email = rosterTeamEmail.trim();
      const teamName = rosterTeamName.trim();

      if (!email && !teamName) {
        throw new Error("Enter the captain email, team name, or both.");
      }
      ...
      let query = supabase
        .from("teams")
        .select("*")
        .eq("league_type", registrationDataLeague)
        .eq("season_key", registrationSeasonKey)
        .order("created_at", { ascending: false });

      if (email) query = query.ilike("captain_email", email);
      if (teamName) query = query.ilike("team_name", `%${teamName}%`);

      const { data, error } = await withTimeout(query.limit(10), 15000);

      if (error) throw new Error(error.message || JSON.stringify(error));
      if (!data?.length) {
        throw new Error("No team was found. Check the captain email, team name, and selected league.");
      }

      const exactTeam =
        data.find((row) => (row.team_name || "").toLowerCase() === teamName.toLowerCase()) ||
        data[0];

      // Payment/invoice status is intentionally not checked here.
      // Captains may reopen and update their roster throughout the season.
      setRosterTeam(mapTeamRow(exactTeam));
    } catch (err) {
      alert(err.message || String(err));
    } finally {
  ...
  async function handleLookupWaiverTeam(e) {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        throw new Error("No team was found with that name. Check spelling or ask your captain to confirm the team was registered.");
      }

      const exactMatch = data.find(
        (team) => (team.team_name || "").toLowerCase() === searchName.toLowerCase()
      );

      setWaiverTeam(mapTeamRow(exactMatch || data[0]));
    } catch (err) {
      alert(err.message || String(err));
    } finally {
  ...
  async function handleKickballPlayerWaiverCheckout() {
    alert("Payments are handled separately by invoice. Please sign the waiver using the Sign Player Waiver button.");
  }
  ...
  async function buildSchedule() { try { const eligibleTeams = adminTeams.filter((team) => team.paidStatus === "Paid"); if (eligibleTeams.length < 2) throw new Error(`You need at least 2 paid ${activeAdminLeague.name} teams to generate a schedule.`); const generated = generateRoundRobinSchedule(eligibleTeams); await withTimeout(supabase.from("schedule_games").delete().eq("league_type", adminDataLeague).eq("season_key", adminSeasonKey).neq("id", "00000000-0000-0000-0000-000000000000"), 15000); const rows = generated.map((g) => ({ week: g.week, home_team_id: g.homeTeamId, away_team_id: g.awayTeamId, game_date: g.date, game_time: g.time, field: activeAdminLeague.location, status: g.status, home_score: null, away_score: null, league_type: adminDataLeague, season_key: adminSeasonKey })); if (rows.length) { const { error } = await withTimeout(supabase.from("schedule_games").insert(rows), 15000); if (error) throw new Error(error.message); } await loadData(); alert(`${activeAdminLeague.name} schedule generated using paid teams only.`); } catch (err) { alert(err.message || String(err)); } }
  ...
  async function saveScheduleGame(gameId) { try { const v = scheduleForm[gameId]; if (!v) throw new Error("No schedule values found."); setSavingScheduleGameId(gameId); const { error } = await supabase.from("schedule_games").update({ week: Number(v.week) || 1, game_date: v.date || "", game_time: v.time || "", field: v.field || activeAdminLeague.location, status: v.status || "Scheduled", home_team_id: v.homeTeamId || null, away_team_id: v.awayTeamId || null, home_score: safeNumber(v.homeScore), away_score: safeNumber(v.awayScore), league_type: adminDataLeague, season_key: adminSeasonKey }).eq("id", gameId); if (error) throw new Error(error.message); await loadData(); alert("Game updated."); } catch (err) { alert(err.message || String(err)); } finally { setSavingScheduleGameId(""); } }
  async function savePlayerStat(e) { e.preventDefault(); try { if (!statForm.playerName.trim()) throw new Error("Enter player name."); if (!statForm.teamId) throw new Error("Select a team."); setSavingStat(true); const payload = { player_name: statForm.playerName.trim(), team_id: statForm.teamId, games_played: Number(statForm.gamesPlayed) || 0, hits: Number(statForm.hits) || 0, home_runs: Number(statForm.homeRuns) || 0, rbis: Number(statForm.rbis) || 0, runs: Number(statForm.runs) || 0, updated_at: new Date().toISOString(), league_type: adminDataLeague, season_key: adminSeasonKey }; const existing = playerStats.find((p) => p.leagueType === adminDataLeague && p.seasonKey === adminSeasonKey && p.teamId === statForm.teamId && p.playerName.toLowerCase() === statForm.playerName.trim().toLowerCase()); const { error } = existing ? await supabase.from("player_stats").update(payload).eq("id", existing.id) : await supabase.from("player_stats").insert([payload]); if (error) throw new Error(error.message); setStatForm(initialStatForm); await loadData(); alert("Player stats saved."); } catch (err) { alert(err.message || String(err)); } finally { setSavingStat(false); } }

  function generateBracketFromStandings(optionKey) { const dataLeague = getLeagueDataKey(optionKey); const seasonKey = getSeasonKey(optionKey); const seeds = computeStandings(teams.filter((t) => t.leagueType === dataLeague && t.seasonKey === seasonKey), schedule.filter((g) => g.leagueType === dataLeague && g.seasonKey === seasonKey)).slice(0, 8); if (seeds.length < 2) return []; const mk = (roundName, gameOrder, a, b) => ({ roundName, gameOrder, seedHome: a.seed, seedAway: b.seed, homeTeamId: a.teamId, awayTeamId: b.teamId, status: "Scheduled", homeScore: "", awayScore: "", winnerTeamId: null, leagueType: dataLeague, seasonKey }); if (seeds.length >= 8) return [mk("Quarterfinal", 1, seeds[0], seeds[7]), mk("Quarterfinal", 2, seeds[3], seeds[4]), mk("Quarterfinal", 3, seeds[1], seeds[6]), mk("Quarterfinal", 4, seeds[2], seeds[5])]; if (seeds.length >= 4) return [mk("Semifinal", 1, seeds[0], seeds[3]), mk("Semifinal", 2, seeds[1], seeds[2])]; return [mk("Championship", 1, seeds[0], seeds[1])]; }
  async function generatePlayoffBracket() { try { const generated = generateBracketFromStandings(adminLeague); if (!generated.length) throw new Error(`Not enough ${activeAdminLeague.name} teams to generate a playoff bracket.`); await optionalQuery(supabase.from("playoff_games").delete().eq("league_type", adminDataLeague).eq("season_key", adminSeasonKey).neq("id", "00000000-0000-0000-0000-000000000000")); const { error } = await supabase.from("playoff_games").insert(generated.map((g) => ({ round_name: g.roundName, seed_home: g.seedHome, seed_away: g.seedAway, home_team_id: g.homeTeamId, away_team_id: g.awayTeamId, game_order: g.gameOrder, home_score: null, away_score: null, winner_team_id: null, status: g.status, league_type: adminDataLeague, season_key: adminSeasonKey }))); if (error) throw new Error(error.message); await loadData(); alert(`${activeAdminLeague.name} playoff bracket generated.`); } catch (err) { alert(err.message || String(err)); } }
  async function savePlayoffGame(gameId, values) { try { setSavingPlayoffGameId(gameId); const hs = safeNumber(values.homeScore); const as = safeNumber(values.awayScore); let winnerTeamId = null; if (hs !== null && as !== null) { if (hs > as) winnerTeamId = values.homeTeamId || null; if (as > hs) winnerTeamId = values.awayTeamId || null; } const { error } = await supabase.from("playoff_games").update({ home_team_id: values.homeTeamId || null, away_team_id: values.awayTeamId || null, home_score: hs, away_score: as, winner_team_id: winnerTeamId, status: values.status || "Scheduled", updated_at: new Date().toISOString(), league_type: adminDataLeague, season_key: adminSeasonKey }).eq("id", gameId); if (error) throw new Error(error.message); await loadData(); alert("Playoff game updated."); } catch (err) { alert(err.message || String(err)); } finally { setSavingPlayoffGameId(""); } }

  function getScheduleWithNames(leagueType, seasonKey) {
    return schedule
      .filter((game) => game.leagueType === leagueType && game.seasonKey === seasonKey)
      .map((game) => ({
        ...game,
      ...
      .sort((a, b) => {
        ...
        const times = { "6:00 PM": 1, "7:00 PM": 2, "8:00 PM": 3, "9:00 PM": 4 };
        return (times[a.time] || 99) - (times[b.time] || 99) || (a.field || "").localeCompare(b.field || "");
      });
  }
  ...
  const homeKickballTeams = teams.filter(
    (team) => team.leagueType === "kickball" && team.seasonKey === "kickball_summer_2026"
  );
  const homeRegisteredPlayers =
    teamPlayers.filter(
      (player) =>
        player.seasonKey === "softball_summer_2026" ||
        player.seasonKey === "kickball_summer_2026"
    ).length +
    freeAgents.filter(
      (player) =>
        player.seasonKey === "softball_summer_2026" ||
  ...
  const homeCompletedGames = schedule.filter(
    (game) =>
      (game.seasonKey === "softball_summer_2026" ||
        game.seasonKey === "kickball_summer_2026") &&
      game.homeScore !== "" &&
      game.awayScore !== ""
  ).length;
  const autoStandings = computeStandings(adminTeams, adminSchedule);

  function getManualStanding(teamId) {
    return manualStandings.find(
      (row) => row.teamId === teamId && row.leagueType === adminDataLeague && row.seasonKey === adminSeasonKey
    );
  }

  const standings = autoStandings
    .map((row) => {
      const manual = getManualStanding(row.teamId);
      return manual
        ? {
            ...row,
            seed: manual.seed === "" ? row.seed : Number(manual.seed),
            wins: Number(manual.wins),
            losses: Number(manual.losses),
            ties: Number(manual.ties),
            runsFor: Number(manual.runsFor),
            runsAgainst: Number(manual.runsAgainst),
            differential: Number(manual.differential),
            gamesPlayed: Number(manual.wins) + Number(manual.losses) + Number(manual.ties),
          }
        : row;
    })
    .sort((a, b) => {
      const aSeed = Number(a.seed) || 999;
      const bSeed = Number(b.seed) || 999;
      return aSeed - bSeed || b.wins - a.wins || b.differential - a.differential;
    });

  function getStandingInput(teamId, field, fallback) {
    const pending = standingForm[teamId];
    if (pending && Object.prototype.hasOwnProperty.call(pending, field)) return pending[field];
    const manual = getManualStanding(teamId);
    if (manual && Object.prototype.hasOwnProperty.call(manual, field)) return manual[field];
    return fallback;
  }

  function handleStandingInputChange(teamId, field, value) {
    setStandingForm((previous) => ({
      ...previous,
      [teamId]: {
        ...(previous[teamId] || {}),
        [field]: value,
  ...
  async function saveManualStanding(team) {
    try {
      setSavingStandingTeamId(team.teamId);

      const wins = Number(getStandingInput(team.teamId, "wins", team.wins)) || 0;
      const losses = Number(getStandingInput(team.teamId, "losses", team.losses)) || 0;
      const ties = Number(getStandingInput(team.teamId, "ties", team.ties)) || 0;
      const runsFor = Number(getStandingInput(team.teamId, "runsFor", team.runsFor)) || 0;
      const runsAgainst = Number(getStandingInput(team.teamId, "runsAgainst", team.runsAgainst)) || 0;const differentialValue = getStandingInput(team.teamId, "differential", runsFor - runsAgainst);
      const differential = differentialValue === "" ? runsFor - runsAgainst : Number(differentialValue);
      const seedValue = getStandingInput(team.teamId, "seed", team.seed);
      const seed = seedValue === "" ? null : Number(seedValue);

      const payload = {
        team_id: team.teamId,
        league_type: adminDataLeague,
        season_key: adminSeasonKey,
        seed,
        wins,
        losses,
        ties,
        ...
        differential: Number.isFinite(differential) ? differential : runsFor - runsAgainst,
        updated_at: new Date().toISOString(),
      };

      const { error } = await supabase
        .from("team_standings")
        .upsert(payload, { onConflict: "team_id,season_key" });

      if (error) throw new Error(error.message || JSON.stringify(error));

      await loadData();
      alert("Standings override saved.");
    } catch (err) {
      alert(err.message || String(err));
    } finally {
      setSavingStandingTeamId("");
    }
  }

  async function clearManualStanding(teamId) {
    try {
      setSavingStandingTeamId(teamId);

      const { error } = await supabase
        .from("team_standings")
        .delete()
        .eq("team_id", teamId)
        .eq("season_key", adminSeasonKey);
      ...
      await loadData();
      alert("Manual override cleared. Standings now use game scores.");
    } catch (err) {
      alert(err.message || String(err));
    } finally {
      setSavingStandingTeamId("");
  ...
  const softballScheduleWithNames = getScheduleWithNames("softball", "softball_spring_2026");
  const softballSummerScheduleWithNames = getScheduleWithNames("softball", "softball_summer_2026");
  const softballFallScheduleWithNames = getScheduleWithNames("softball", "softball_fall_softball_19578");
  const kickballScheduleWithNames = getScheduleWithNames("kickball", "kickball_summer_2026");
  const adminScheduleWithNames = getScheduleWithNames(adminDataLeague, adminSeasonKey);
  const softballBracket = playoffGames.filter((g) => g.leagueType === "softball" && g.seasonKey === "softball_spring_2026").length ? playoffGames.filter((g) => g.leagueType === "softball" && g.seasonKey === "softball_spring_2026") : generateBracketFromStandings("softball");
  const softballSummerBracket = playoffGames.filter((g) => g.leagueType === "softball" && g.seasonKey === "softball_summer_2026").length ? playoffGames.filter((g) => g.leagueType === "softball" && g.seasonKey === "softball_summer_2026") : generateBracketFromStandings("softballSummer");
  const kickballBracket = playoffGames.filter((g) => g.leagueType === "kickball" && g.seasonKey === "kickball_summer_2026").length ? playoffGames.filter((g) => g.leagueType === "kickball" && g.seasonKey === "kickball_summer_2026") : generateBracketFromStandings("kickball");
    ...
    <div>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .home-banner-card { overflow: hidden; border-radius: 22px 22px 0 0; background: #1a100b; box-shadow: 0 14px 34px rgba(0,0,0,.18); }
        .home-banner-image { display: block; width: 100%; height: auto; object-fit: cover; }
        .hero-copy { max-width: 720px; }
        .home-pills { margin-top: 18px; }
        .photo-section { margin: 34px 0; }
        .section-heading-row { display: flex; align-items: end; justify-content: space-between; gap: 24px; margin-bottom: 18px; }
        .section-heading-row .section-subtitle { max-width: 520px; margin-bottom: 4px; }
        .home-photo-grid { display: grid; grid-template-columns: 1.35fr .85fr; grid-template-rows: 260px 260px; gap: 16px; }
        .home-photo-card { margin: 0; overflow: hidden; border-radius: 18px; background: #111; box-shadow: 0 12px 26px rgba(0,0,0,.16); }
        .home-photo-card img { width: 100%; height: 100%; display: block; object-fit: cover; transition: transform .35s ease; }
        .home-photo-card:hover img { transform: scale(1.025); }
        .home-photo-card-1 { grid-row: 1 / span 2; }
        .standings-header { display: flex; align-items: end; justify-content: space-between; gap: 20px; flex-wrap: wrap; margin-bottom: 18px; }
        .standings-select-wrap { min-width: 260px; display: grid; gap: 8px; font-weight: 800; }
        .standings-select-wrap select { background: white; }
        .standings-selected-title { font-size: 24px; font-weight: 900; margin: 0 0 12px; }
        .home-page { max-width: 1280px; padding-top: 24px; }
        .home-hero-shell { position: relative; overflow: hidden; border-radius: 26px; min-height: 560px; background: #160d08; box-shadow: 0 22px 55px rgba(32,18,9,.24); }
        .home-banner-image { width: 100%; height: 100%; min-height: 560px; display: block; object-fit: cover; object-position: center; }
        .home-hero-overlay { position: absolute; inset: 0; display: flex; align-items: flex-end; padding: 42px; background: linear-gradient(90deg, rgba(14,8,5,.94) 0%, rgba(14,8,5,.72) 45%, rgba(14,8,5,.12) 78%), linear-gradient(0deg, rgba(14,8,5,.70), transparent 60%); }
        .home-hero-content { max-width: 720px; color: white; }
        .home-hero-content h1 { margin: 10px 0 16px; font-size: clamp(44px, 6vw, 78px); line-height: .95; color: white; letter-spacing: -2px; }
        .home-hero-content p { max-width: 650px; margin: 0; font-size: 18px; line-height: 1.75; color: rgba(255,255,255,.90); }
        .home-eyebrow, .home-section-kicker { display: inline-block; color: #f59e0b; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; font-size: 12px; }
        .home-main-cta { box-shadow: 0 10px 24px rgba(245,158,11,.25); }
        .home-secondary-cta { background: rgba(255,255,255,.10); border-color: rgba(255,255,255,.32); color: white; }
        .home-stats-strip { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin: -34px 28px 42px; position: relative; z-index: 3; }
        .home-stat-card { background: white; border-radius: 18px; padding: 20px; box-shadow: 0 14px 32px rgba(30,16,7,.14); }
        .home-stat-card strong { display: block; margin: 8px 0 4px; font-size: 34px; }
        .home-stat-label { display: block; font-size: 13px; font-weight: 900; color: #7c4b1d; text-transform: uppercase; letter-spacing: .06em; }
        .home-stat-card small { color: #75685c; }
        .home-registration-section, .photo-section, .home-features-section { margin: 58px 0; }
        .home-league-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        .home-league-card { position: relative; min-height: 380px; overflow: hidden; border-radius: 22px; padding: 28px; color: white; box-shadow: 0 18px 38px rgba(28,14,5,.18); }
        .home-league-card::before { content: ""; position: absolute; inset: 0; background: linear-gradient(145deg, rgba(20,12,7,.97), rgba(20,12,7,.72)); }
        .home-league-card-softball { background: linear-gradient(135deg, #5d2c0e, #c66a14); }
        .home-league-card-kickball { background: linear-gradient(135deg, #171717, #49311c); }
        .home-league-card-content { position: relative; z-index: 1; }
        .home-league-card h3 { margin: 14px 0 8px; font-size: 32px; color: white; }
        .home-league-card p, .home-league-card li { color: rgba(255,255,255,.88); line-height: 1.65; }
        .home-league-badge { display: inline-block; padding: 7px 11px; border-radius: 999px; background: rgba(245,158,11,.18); color: #ffd99b; font-weight: 900; }
        .home-photo-card { position: relative; }
        .home-photo-card figcaption { position: absolute; left: 0; right: 0; bottom: 0; padding: 45px 18px 16px; color: white; font-weight: 900; font-size: 18px; background: linear-gradient(transparent, rgba(0,0,0,.82)); }
        .home-feature-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; }
        .home-feature-card { background: white; border-radius: 18px; padding: 22px; box-shadow: 0 12px 28px rgba(35,19,8,.09); border: 1px solid #ece3d7; }
        .home-feature-icon { display: inline-grid; place-items: center; width: 42px; height: 42px; border-radius: 12px; background: #1b120d; color: #f59e0b; font-weight: 900; }
        .home-sponsor-section { display: grid; grid-template-columns: 1.3fr .7fr; gap: 30px; align-items: center; margin: 60px 0 30px; padding: 36px; border-radius: 24px; color: white; background: linear-gradient(135deg, #17100c, #3e2514); }
        .home-sponsor-copy h2 { margin: 8px 0; font-size: 42px; color: white; }
        .home-sponsor-tags { display: flex; gap: 8px; flex-wrap: wrap; }
        .home-sponsor-tags span { padding: 7px 10px; border: 1px solid rgba(255,255,255,.20); border-radius: 999px; }
        .home-sponsor-offer { text-align: center; padding: 28px; border-radius: 18px; background: #f59e0b; color: #211309; }
        .home-sponsor-offer strong { display: block; font-size: 52px; }
        .home-footer { display: grid; grid-template-columns: 1.2fr 1fr 1fr; gap: 30px; margin-top: 30px; padding: 34px; border-radius: 22px 22px 0 0; color: white; background: #111; }
        .home-footer-links { display: flex; gap: 8px; flex-wrap: wrap; }
        .home-footer-links button { background: transparent; color: white; }
        .home-footer-contact { display: grid; gap: 8px; }
        ...
        @media (max-width: 980px) {
          .hero, .promo-grid, .grid2, .grid3 { grid-template-columns: 1fr; }
          .home-hero-shell, .home-banner-image { min-height: 620px; }
          .home-hero-overlay { padding: 28px; background: linear-gradient(0deg, rgba(14,8,5,.96), rgba(14,8,5,.34)); }
          .home-stats-strip { grid-template-columns: repeat(2, 1fr); margin: -24px 14px 38px; }
          .home-league-grid, .home-feature-grid, .home-sponsor-section, .home-footer { grid-template-columns: 1fr; }
          .home-photo-grid { grid-template-columns: 1fr; grid-template-rows: auto; }
          .home-photo-card-1 { grid-row: auto; }
          .home-photo-card { min-height: 280px; }
          .section-heading-row { align-items: flex-start; flex-direction: column; }
        }
        @media (max-width: 620px) {
          .home-page { padding-left: 10px; padding-right: 10px; }
          .home-hero-shell, .home-banner-image { min-height: 680px; }
          .home-hero-overlay { padding: 22px; }
          .hero-actions { display: grid; grid-template-columns: 1fr; }
          .home-stats-strip { grid-template-columns: 1fr 1fr; gap: 10px; margin-left: 8px; margin-right: 8px; }
          .home-stat-card { padding: 15px; }
      ...
      {page === "home" && (
        <div className="page home-page">
          <section className="home-hero-shell">
            <img src={logoImage} alt="Desert Rec West Valley Softball" className="home-banner-image" />
            <div className="home-hero-overlay">
              <div className="home-hero-content">
                <span className="home-eyebrow">Buckeye • West Valley • Adult Recreation</span>
                <h1>Where the West Valley Comes to Play</h1>
                <p>
                  Organized adult softball and kickball with online registration,
                  live schedules, standings, rosters, waivers, playoffs, and
                  championship nights at Sundance Park.
                </p>
                <div className="hero-actions">
                  <button className="primary home-main-cta" onClick={() => openRegistrationForLeague("softballSummer")}>
                    Register Summer Softball
                  </button>
                  <button className="home-secondary-cta" onClick={() => openRegistrationForLeague("kickball")}>
                    Register Kickball
                  </button>
                  <button className="home-secondary-cta" onClick={() => setPage("schedule")}>
                    View Schedule
                  </button>
                </div>
              </div>
          ...
          <section className="home-stats-strip">
            <article className="home-stat-card"><span className="home-stat-label">Summer Softball Teams</span><strong>{homeSummerSoftballTeams.length}</strong><small>Thursday nights</small></article>
            <article className="home-stat-card"><span className="home-stat-label">Kickball Teams</span><strong>{homeKickballTeams.length}</strong><small>Sunday evenings</small></article>
            <article className="home-stat-card"><span className="home-stat-label">Registered Players</span><strong>{homeRegisteredPlayers}</strong><small>Current active seasons</small></article>
            <article className="home-stat-card"><span className="home-stat-label">Completed Games</span><strong>{homeCompletedGames}</strong><small>Scores posted online</small></article>
          </section>

          <section className="home-registration-section">
            <div className="section-heading-row">
              <div><span className="home-section-kicker">Registration Open</span><h2 className="section-title">Choose Your League</h2></div>
              <p className="section-subtitle">Register a full team, join as a free agent, manage your roster, and complete player waivers online.</p>
            </div>
            <div className="home-league-grid">
              <article className="home-league-card home-league-card-softball">
                <div className="home-league-card-content">
                  <span className="home-league-badge">Softball</span>
                  <h3>Summer Coed Softball</h3>
                  <p>Thursday nights at Sundance Park in Buckeye.</p>
                  <ul><li>Team and free-agent registration</li><li>Online roster and waiver management</li><li>Weekly schedules, standings, and playoffs</li></ul>
                  <button className="primary" onClick={() => openRegistrationForLeague("softballSummer")}>Register for Softball</button>
                </div>
              </article>
              <article className="home-league-card home-league-card-kickball">
                <div className="home-league-card-content">
                  <span className="home-league-badge">Kickball</span>
                  <h3>Sunday Evening Kickball</h3>
                  <p>A social coed league with real competition and a relaxed atmosphere.</p>
                  <ul><li>Bring a team or register individually</li><li>Digital player waivers</li><li>Live schedules, standings, and playoff bracket</li></ul>
                  <button className="primary" onClick={() => openRegistrationForLeague("kickball")}>Register for Kickball</button>
                </div>
              </article>
            </div>
          </section>
          ...
          <section className="photo-section">
            <div className="section-heading-row">
              <div><span className="home-section-kicker">League Life</span><h2 className="section-title">Competition. Community. Game Night.</h2></div>
              <p className="section-subtitle">Real Desert Rec athletes, real game action, and championship moments.</p>
            </div>
            <div className="home-photo-grid">
              {homeGalleryImages.map((image, index) => (
                <figure className={`home-photo-card home-photo-card-${index + 1}`} key={image.src}>
                  <img src={image.src} alt={image.alt} />
                  <figcaption>{index === 0 ? "Thursday Night Action" : index === 1 ? "Play Hard. Play Safe." : "Championship Moments"}</figcaption>
                </figure>
              ))}
            </div>
          </section>
          ...
          <section className="home-features-section">
            <div className="section-heading-row"><div><span className="home-section-kicker">Everything in One Place</span><h2 className="section-title">Built for Players and Captains</h2></div></div>
            <div className="home-feature-grid">
              <article className="home-feature-card"><span className="home-feature-icon">01</span><h3>Simple Registration</h3><p>Register a team or join as a free agent from any phone or computer.</p></article>
              <article className="home-feature-card"><span className="home-feature-icon">02</span><h3>Online Waivers</h3><p>Players search by team name, select their name, and sign digitally.</p></article>
              <article className="home-feature-card"><span className="home-feature-icon">03</span><h3>Live Schedule</h3><p>See weekly matchups, fields, game times, scores, and game status.</p></article>
              <article className="home-feature-card"><span className="home-feature-icon">04</span><h3>Current Standings</h3><p>Choose a sport and follow wins, losses, runs, differential, and seed.</p></article>
            </div>
          </section>

          <section className="home-sponsor-section">
            <div className="home-sponsor-copy">
              <span className="home-section-kicker">Official League Sponsor</span>
              <h2>Tailgaters</h2>
              <p>Join the league after the games and mention Desert Rec Sports League for the current game-day special.</p>
              <div className="home-sponsor-tags"><span>Game-Day Specials</span><span>West Valley Community</span><span>Postgame Headquarters</span></div>
            </div>
            <div className="home-sponsor-offer"><strong>10% OFF</strong><span>Full-priced food and drinks on league game days</span></div>
          </section>

          <footer className="home-footer">
            <div><strong>Desert Rec Sports League</strong><p>Adult softball and kickball serving Buckeye and the West Valley.</p></div>
            <div className="home-footer-links"><button onClick={() => setPage("softballRegister")}>Registration</button><button onClick={() => setPage("schedule")}>Schedule</button><button onClick={() => setPage("standings")}>Standings</button><button onClick={() => setPage("waiver")}>Waiver</button></div>
            <div className="home-footer-contact"><span>desertrecsportsleague@gmail.com</span><span>desertrecsportsleague.com</span></div>
          </footer>
        </div>
      )}
      ...
      {page === "schedule" && <div className="page"><h2 className="section-title">Current Softball Schedule</h2><div className="league-switch"><button className="active">Current Softball</button><button onClick={() => setPage("summerSoftballSchedule")}>Summer Softball</button><button onClick={() => setPage("fallSoftballSchedule")}>Fall Softball 2026</button><button onClick={() => setPage("kickballSchedule")}>Kickball Schedule</button></div><ScheduleTable games={softballScheduleWithNames} /></div>}
      {page === "fallSoftballSchedule" && <div className="page"><h2 className="section-title">Fall Softball 2026 Schedule</h2><div className="league-switch"><button onClick={() => setPage("schedule")}>Current Softball</button><button onClick={() => setPage("summerSoftballSchedule")}>Summer Softball</button><button className="active">Fall Softball 2026</button><button onClick={() => setPage("kickballSchedule")}>Kickball Schedule</button></div><ScheduleTable games={softballFallScheduleWithNames} /></div>}
      {page === "summerSoftballSchedule" && <div className="page"><h2 className="section-title">New Softball Schedule - Starts July 16</h2><div className="league-switch"><button onClick={() => setPage("schedule")}>Current Softball</button><button className="active">New Softball - July 16</button><button onClick={() => setPage("kickballSchedule")}>Kickball Schedule</button></div><ScheduleTable games={softballSummerScheduleWithNames} /></div>}
        ...
        <div className="page">
          <div className="standings-header">
            <div>
              ...
              <p className="section-subtitle">Choose a league to view only that season’s standings.</p>
            </div>
            <label className="standings-select-wrap">
              <span>League</span>
          ...
          <div className="standings-selected-title">{selectedStandingsTitle}</div>
          <StandingsTable list={selectedStandings} />
        </div>
      )}
      {page === "playoffs" && <div className="page"><h2 className="section-title">Playoffs</h2><div className="grid3"><div><h3>Current Softball</h3><PlayoffTable games={softballBracket} /></div><div><h3>New Softball - July 16</h3><PlayoffTable games={softballSummerBracket} /></div><div><h3>Kickball</h3><PlayoffTable games={kickballBracket} /></div></div></div>}
      ...
      {page === "admin" && <div className="page">{authLoading ? <div className="card">Loading admin login...</div> : !user || user.email !== ADMIN_EMAIL ? <div className="card" style={{ maxWidth: 520 }}><h2>Admin Login</h2><form noValidate onSubmit={handleAdminLogin} className="form"><input value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="Admin email" /><input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Password" /><button type="submit" className="primary">Sign In</button></form></div> : <><div className="row" style={{ justifyContent: "space-between", marginBottom: 20 }}><h2 className="section-title" style={{ margin: 0 }}>Admin Dashboard</h2><button onClick={handleLogout}>Sign Out</button></div><div className="card"><h3>Admin League Control</h3><div className="league-switch"><button className={adminLeague === "softball" ? "active" : ""} onClick={() => setAdminLeague("softball")}>Current Softball Admin</button><button className={adminLeague === "softballSummer" ? "active" : ""} onClick={() => setAdminLeague("softballSummer")}>Summer Softball Admin</button><button className={adminLeague === "softballFall" ? "active" : ""} onClick={() => setAdminLeague("softballFall")}>Fall Softball 2026 Admin</button><button className={adminLeague === "kickball" ? "active" : ""} onClick={() => setAdminLeague("kickball")}>Kickball Admin</button></div><p className="section-subtitle" style={{ marginBottom: 0 }}>Current admin view: <strong>{activeAdminLeague.name}</strong></p></div><div className="grid3"><div className="card"><div>{activeAdminLeague.name} Teams</div><div className="stat">{adminTeams.length}</div></div><div className="card"><div>{activeAdminLeague.name} Free Agents</div><div className="stat">{adminFreeAgents.length}</div></div><div className="card"><div>Open Team Spots</div><div className="stat">{Math.max(activeAdminLeague.teamCap - adminTeams.length, 0)}</div></div></div><div className="card"><div className="row"><button className="primary" onClick={buildSchedule}>Generate {activeAdminLeague.name} Round Robin</button><button className="primary" disabled={assigningFreeAgents} onClick={assignPaidFreeAgentsToTeam}>{assigningFreeAgents ? "Assigning..." : `Assign ${activeAdminLeague.name} Paid Free Agents`}</button><button className="primary" disabled={creatingNewFreeAgentTeam} onClick={handleCreateNewFreeAgentTeam}>{creatingNewFreeAgentTeam ? "Creating..." : `Create New ${activeAdminLeague.name} Free Agent Team`}</button><button className="primary" onClick={generatePlayoffBracket}>Generate {activeAdminLeague.name} Playoff Bracket</button></div></div>
        ...
        <div className="card">
          <h3>{activeAdminLeague.name} Standings Control</h3>
          <p className="section-subtitle">
            Enter weekly game scores above and save each game to update standings automatically. You may also override any team below. Use “Use Auto” to return that team to score-based standings.
          </p>
          {standings.length === 0 ? <p>No standings available yet.</p> : (
            <table>
              <thead>
                <tr>
                  <th>Seed</th><th>Team</th><th>GP</th><th>W</th><th>L</th><th>T</th><th>RF</th><th>RA</th><th>Diff</th><th>Save</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((team) => (
                  <tr key={team.teamId}>
                    <td><input className="mini-input" type="number" min="1" value={getStandingInput(team.teamId, "seed", team.seed)} onChange={(e) => handleStandingInputChange(team.teamId, "seed", e.target.value)} /></td>
                    <td>{team.teamName}</td>
                    <td>{Number(getStandingInput(team.teamId, "wins", team.wins) || 0) + Number(getStandingInput(team.teamId, "losses", team.losses) || 0) + Number(getStandingInput(team.teamId, "ties", team.ties) || 0)}</td>
                    <td><input className="mini-input" type="number" min="0" value={getStandingInput(team.teamId, "wins", team.wins)} onChange={(e) => handleStandingInputChange(team.teamId, "wins", e.target.value)} /></td>
                    <td><input className="mini-input" type="number" min="0" value={getStandingInput(team.teamId, "losses", team.losses)} onChange={(e) => handleStandingInputChange(team.teamId, "losses", e.target.value)} /></td>
                    <td><input className="mini-input" type="number" min="0" value={getStandingInput(team.teamId, "ties", team.ties)} onChange={(e) => handleStandingInputChange(team.teamId, "ties", e.target.value)} /></td>
                    <td><input className="mini-input" type="number" min="0" value={getStandingInput(team.teamId, "runsFor", team.runsFor)} onChange={(e) => handleStandingInputChange(team.teamId, "runsFor", e.target.value)} /></td>
                    <td><input className="mini-input" type="number" min="0" value={getStandingInput(team.teamId, "runsAgainst", team.runsAgainst)} onChange={(e) => handleStandingInputChange(team.teamId, "runsAgainst", e.target.value)} /></td>
                    <td><input className="mini-input" type="number" value={getStandingInput(team.teamId, "differential", team.differential)} onChange={(e) => handleStandingInputChange(team.teamId, "differential", e.target.value)} /></td>
                    <td>
                      <div className="row">
                        <button className="primary" disabled={savingStandingTeamId === team.teamId} onClick={() => saveManualStanding(team)}>
                          {savingStandingTeamId === team.teamId ? "Saving..." : "Save"}
                        </button>
                        <button disabled={savingStandingTeamId === team.teamId} onClick={() => clearManualStanding(team.teamId)}>Use Auto</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="card"><h3>Roster Players</h3><table><thead><tr><th>Team</th><th>Name</th><th>Email</th><th>Waiver</th></tr></thead><tbody>{teamPlayers.filter((p) => p.leagueType === adminDataLeague && p.seasonKey === adminSeasonKey).map((player) => <tr key={player.id}><td>{teamNameById(player.teamId)}</td><td>{player.playerName}</td><td>{player.playerEmail || "—"}</td><td><StatusBadge status={player.signedWaiver ? "Signed" : "Missing"} /></td></tr>)}</tbody></table></div></>}</div>}
    </div>
  );
}
