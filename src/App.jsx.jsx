...
function getTeamStatus(team) { if (team.paidStatus === "Paid") return "Paid"; if (getPaymentDeadlinePassed()) return "Locked"; return "Pending"; }
function getFreeAgentStatus(player) { if (player.paidStatus === "Paid") return "Paid"; if (getPaymentDeadlinePassed()) return "Cut"; return "Pending"; }
...
function mapTeamPlayerRow(row) { return { id: row.id, teamId: row.team_id, playerName: row.player_name, playerEmail: row.player_email || "", playerPhone: row.player_phone || "", signedWaiver: !!row.signed_waiver, signatureName: row.signature_name || "", signedAt: row.signed_at || "", leagueType: normalizeLeague(row.league_type) }; }
...
async function handleFreeAgentCheckout(e) { e.preventDefault(); if (savingFreeAgent || startingFreeAgentCheckout) return; setStartingFreeAgentCheckout(true); try { const saved = await saveFreeAgentRegistration(); const response = await withTimeout(fetch("/api/create-free-agent-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ freeAgentId: saved.id, playerName: saved.player_name, playerEmail: saved.player_email, leagueType: registrationLeague }) }), 15000); const result = await response.json(); if (!response.ok) throw new Error(result.error || "Unable to start free agent checkout."); window.location.href = result.url; } catch (err) { alert(err.message || String(err)); } finally { setStartingFreeAgentCheckout(false); } }
...
async function handleResumeFreeAgentCheckout() { try { if (!foundFreeAgent) throw new Error("No saved free agent selected."); if (foundFreeAgent.paidStatus === "Paid") throw new Error("This free agent is already paid."); setResumeFreeAgentCheckoutLoading(true); const response = await withTimeout(fetch("/api/create-free-agent-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ freeAgentId: foundFreeAgent.id, playerName: foundFreeAgent.playerName, playerEmail: foundFreeAgent.playerEmail, leagueType: registrationLeague }) }), 15000); const result = await response.json(); if (!response.ok) throw new Error(result.error || "Unable to start free agent checkout."); window.location.href = result.url; } catch (err) { alert(err.message || String(err)); } finally { setResumeFreeAgentCheckoutLoading(false); } }
...
if (waiverTeam.leagueType !== "kickball") throw new Error("This payment option is for kickball players only.");
...
if (waiverError) throw new Error(waiverError.message || JSON.stringify(waiverError));
...
if (!response.ok) throw new Error(result.error || "Unable to start kickball player checkout.");
