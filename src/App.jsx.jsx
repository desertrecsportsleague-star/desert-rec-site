...
function App() {
  ...
  useEffect(() => {
    setTeams(loadStorage("drsl_teams", []));
    setFreeAgents(loadStorage("drsl_free_agents", []));
    setSchedule(loadStorage("drsl_schedule", []));
  }, []);

  useEffect(() => {
    localStorage.setItem("drsl_teams", JSON.stringify(teams));
  }, [teams]);

  useEffect(() => {
    localStorage.setItem("drsl_free_agents", JSON.stringify(freeAgents));
  }, [freeAgents]);

  useEffect(() => {
    localStorage.setItem("drsl_schedule", JSON.stringify(schedule));
  }, [schedule]);
  ...
  function handleTeamSubmit(e) {
    ...
    const newTeam = {
      id: uid("team"),
      ...teamForm,
      leagueInterest: "Thursday Coed League",
    ...
    alert("Team registration saved in admin dashboard.");
  }

  function handleFreeAgentSubmit(e) {
    e.preventDefault();
    ...
    alert("Free agent registration saved in admin dashboard.");
  }
    ...
    setFreeAgents((prev) =>
      prev.map((player) =>
        player.assignedTeamId === teamId
          ? { ...player, assignedTeamId: "", assignedTeamName: "" }
          : player
      )
    );
    setSchedule((prev) =>
  ...
  function updateFreeAgent(playerId, updates) {
    setFreeAgents((prev) =>
  ...
  function deleteFreeAgent(playerId) {
    ...
    if (!confirmed) return;
    setFreeAgents((prev) => prev.filter((player) => player.id !== playerId));
  }

  function assignFreeAgent(playerId, teamId) {
    const team = teamMap[teamId];
    if (!team) return;
    setFreeAgents((prev) =>
      prev.map((player) =>
        player.id === playerId
          ? { ...player, assignedTeamId: team.id, assignedTeamName: team.teamName }
          : player
      )
    );
  }

  function unassignFreeAgent(playerId) {
    setFreeAgents((prev) =>
      prev.map((player) =>
        player.id === playerId
          ? { ...player, assignedTeamId: "", assignedTeamName: "" }
          ...
          <div className="container">
            <div className="stats-grid">
              <div className="card">
                ...
                <div className="stat-value">Sundance Park</div>
              </div>
              <div className="card">
                <div className="stat-label">Format</div>
                <div className="stat-value">Coed 7 / 3</div>
              </div>
              <div className="card">
              ...
              <div className="card">
                ...
                <div className="stat-value">{spotsLeft} Spots Left</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="section" style={{ paddingTop: 0 }}>
          <div className="container">
            <div style={{ maxWidth: "760px", marginBottom: "28px" }}>
              <div className="section-label">Now Open</div>
              <h2 className="section-title">Launch league now, more leagues coming soon</h2>
              <p className="section-copy">
                We are starting with our Thursday night coed league in Buckeye,
                then expanding into additional men&apos;s and coed offerings as the
                league grows.
              </p>
            </div>
            ...
            <div className="leagues-grid">
              <div className="card">
                <div className="league-location">Now Registering</div>
                <h3 className="league-title">Thursday Coed League</h3>
                <div className="league-copy">
                  A social and competitive coed softball league designed for adult
                  players looking for a well-run Thursday night experience in Buckeye.
                </div>
                <div className="league-format">
                  <strong>Season:</strong> 8-week season at Sundance Park with championship awards.
                </div>
                <div className="league-note">
                  Current registration is open for teams and free agents.
                </div>
              </div>

              <div className="card">
                <div className="league-location">Coming Soon</div>
                <h3 className="league-title">Men&apos;s Softball League</h3>
                <div className="league-copy">
                  A future league built for teams looking for a more competitive
                  men&apos;s slowpitch environment in the West Valley.
                </div>
                <div className="league-format">
                  <strong>Status:</strong> Future league offering.
                </div>
                <div className="league-note">
                  More details will be released after the first season launch.
                </div>
              </div>

              <div className="card">
                <div className="league-location">Coming Soon</div>
                <h3 className="league-title">Additional Coed Nights</h3>
                <div className="league-copy">
                  Future coed divisions and additional weeknight options are planned
                  as demand grows and field access expands.
                </div>
                <div className="league-format">
                  <strong>Status:</strong> Future league offering.
                </div>
                <div className="league-note">
                  Join now and stay connected for new season announcements.
                </div>
              </div>
            </div>
          </div>
            ...
            <div>
              <form className="form-card" onSubmit={handleTeamSubmit}>
                <div className="form-grid">
                  <h3 style={{ margin: 0 }}>Register a Team</h3>
                  ...
                  <select
                    className="select"
                    value={teamForm.paymentTier}
                    onChange={(e) => setTeamForm({ ...teamForm, paymentTier: e.target.value })}
                  >
                    <option value="early">Early Bird ($550)</option>
                    <option value="regular">Regular ($600)</option>
                    <option value="late">Late ($650)</option>
                  </select>

                  <textarea
                    className="textarea"
                    readOnly
                    value={captainWaiverSummary}
                  />
                  ...
                  <label className="checkbox-row">
                    <input
                      ...
                      onChange={(e) => setTeamForm({ ...teamForm, agreeWaiver: e.target.checked })}
                      required
                    />
                  ...
                  <label className="checkbox-row">
                    <input
                      ...
                      onChange={(e) => setTeamForm({ ...teamForm, agreeCaptainDuty: e.target.checked })}
                      required
                    />
                    <span>
                  ...
                  <label className="checkbox-row">
                    <input
                      ...
                      onChange={(e) => setTeamForm({ ...teamForm, agreeElectronic: e.target.checked })}
                      required
                    />
                    <span>
                      I intend my typed name to serve as my electronic signature.
                    </span>
                  </label>

                  <div className="button-row">
                    <button type="submit" className="btn btn-primary">Save Team Registration</button>
                    <a href={TEAM_PAYMENT_LINKS[teamForm.paymentTier]} className="btn btn-secondary dark-outline">
                      Go to Team Payment
                    </a>
                  </div>
                </div>
              ...
              <form className="form-card" style={{ marginTop: "20px" }} onSubmit={handleFreeAgentSubmit}>
                <div className="form-grid">
                  <h3 style={{ margin: 0 }}>Join as a Free Agent</h3>

                  <input
                    className="input"
                    type="text"
                    value={freeAgentForm.playerName}
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, playerName: e.target.value })}
                  ...
                  <input
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, playerEmail: e.target.value })}
                    placeholder="Email Address"
                    required
                  />
                  ...
                  <input
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, playerPhone: e.target.value })}
                    placeholder="Phone Number"
                  />
                  ...
                  <select
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, gender: e.target.value })}
                    required
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, skill: e.target.value })}
                  >
                  ...
                  <input
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, preferredPosition: e.target.value })}
                  ...
                  <input
                    ...
                    onChange={(e) => setFreeAgentForm({ ...freeAgentForm, signature: e.target.value })}
                    placeholder="Type Full Legal Name as Electronic Signature"
                    required
                  />
                  ...
                  <label className="checkbox-row">
                    <input
                      ...
                      onChange={(e) => setFreeAgentForm({ ...freeAgentForm, agreeWaiver: e.target.checked })}
                      required
                    />
                    <span>I have read and agree to the full waiver and release of liability.</span>
                  </label>
                  ...
                  <label className="checkbox-row">
                    <input
                      ...
                      onChange={(e) => setFreeAgentForm({ ...freeAgentForm, agreeElectronic: e.target.checked })}
                      required
                    />
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="card" style={{ marginTop: "24px" }}>
            <h3 style={{ marginTop: 0 }}>League Reminder</h3>
            <p className="section-copy" style={{ maxWidth: "100%" }}>
              Captains are responsible for communicating these rules to all rostered,
              substitute, replacement, and guest players. Desert Rec Sports League may
              clarify or update rules during the season to protect safety, fairness,
      ...
      <section className="section">
        <div className="container">
          <div style={{ maxWidth: "760px", marginBottom: "28px" }}>
            ...
            <p className="section-copy">
              Schedule appears here once created in the admin dashboard.
            </p>
          </div>
          ...
          {weeks.length === 0 ? (
            ...
            weeks.map((week) => (
              <div className="card" key={week} style={{ marginBottom: "20px" }}>
                <h3 style={{ marginTop: 0 }}>Week {week}</h3>
                <div className="table-wrap">
                  <table className="admin-table">
                    <thead>
                      <tr>
      ...
      <section className="section">
        <div className="container">
          <div style={{ maxWidth: "760px", marginBottom: "28px" }}>
            ...
            <p className="section-copy">
              This admin dashboard is stored in your browser for season one. It lets
              you track registrations, payments, free agent assignments, and schedule setup.
            </p>
          </div>
          ...
          <div className="stats-grid" style={{ marginBottom: "24px" }}>
            ...
            <div className="card">
              ...
              <div className="stat-value">{spotsLeft}</div>
            </div>
          </div>

          <div className="card" style={{ marginBottom: "24px" }}>
            <div className="admin-header-row">
              <h3 style={{ margin: 0 }}>Schedule Generator</h3>
              <div className="button-row">
            ...
            <p className="league-note" style={{ marginTop: "12px" }}>
              Generate after your team list is ready. This creates one round-robin matchup cycle.
            </p>
          </div>
          ...
          <div className="card" style={{ marginBottom: "24px" }}>
            <h3 style={{ marginTop: 0 }}>Free Agents</h3>
            {freeAgents.length === 0 ? (
          ...
          <div className="card" style={{ marginBottom: "24px" }}>
            <h3 style={{ marginTop: 0 }}>Team Rosters from Assigned Free Agents</h3>
            {teams.length === 0 ? (
              <div className="league-note">No teams available.</div>
            ) : (
              <div className="roster-grid">
                {teams.map((team) => {
                  ...
                  return (
                    <div key={team.id} className="card" style={{ padding: "20px" }}>
                      <h4 style={{ marginTop: 0, marginBottom: "8px" }}>{team.teamName}</h4>
                      <div className="mini-muted" style={{ marginBottom: "10px" }}>
                        Captain: {team.captainName}
                      </div>
                      <div className="mini-muted" style={{ marginBottom: "10px" }}>
                        Assigned Free Agents: {roster.length} | Male: {maleCount} | Female: {femaleCount}
                      </div>
                      {roster.length === 0 ? (
                        <div className="league-note">No assigned players yet.</div>
                      ) : (
                        <ul className="simple-list">
                          {roster.map((player) => (
                            <li key={player.id}>
                              {player.playerName} — {player.gender || "N/A"} — {player.preferredPosition || "No position"}
                            </li>
                          ))}
                        </ul>
                      )}
          ...
          <div className="card">
            <h3 style={{ marginTop: 0 }}>Schedule Editor</h3>
            {schedule.length === 0 ? (
    ...
    <div className="site">
      <style>{`
        * { box-sizing: border-box; }
        ...
        .stats-grid, .leagues-grid, .features-grid, .roster-grid, .rules-grid, .rules-highlights {
          display: grid;
          gap: 20px;
        }
        ...
        .card, .form-card, .waiver-page-card {
          background: white;
          border-radius: 22px;
          padding: 28px;
        ...
        .stat-label, .section-label, .league-location {
          text-transform: uppercase;
          letter-spacing: 1px;
          font-weight: 800;
          font-size: 13px;
        ...
        .info-box-title { font-weight: 800; margin-bottom: 10px; }
        .info-line { line-height: 1.7; color: #4f473f; font-weight: 600; }
        .email-link {
          color: #b45309;
          text-decoration: none;
          font-weight: 700;
        ...
        .waiver-box-title { font-weight: 800; margin-bottom: 8px; }
        .waiver-box p {
          margin: 0 0 10px;
          color: #5c5147;
          line-height: 1.7;
        ...
        @media (max-width: 980px) {
          .hero-inner, .waitlist-grid, .stats-grid, .leagues-grid, .features-grid, .roster-grid, .rules-grid, .rules-highlights {
