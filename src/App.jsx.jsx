...
/*
Pricing schedule:
- Early Bird ends on April 15, 2026
- Regular ends on May 5, 2026
- After that, Late pricing applies automatically

If you want to adjust later, only change these two dates.
*/
...
if (!data?.user) {
throw new Error("Unable to verify admin user.");
}
...
throw new Error("You need at least 2 paid teams to generate a schedule.");
}
...
<div className="grid2">
<div className="card">
<h2 className="section-title">Why join this league?</h2>
<p className="section-subtitle">
Desert Rec is designed for adults looking for organized games, a
smooth registration process, fair rules, and a fun Thursday night
atmosphere in Buckeye.
</p>
<ul>
<li>Structured weekly schedule</li>
<li>Coed recreational format</li>
<li>Simple registration and payment</li>
<li>Championship recognition</li>
<li>Admin-managed schedule generation</li>
</ul>
</div>
...
<p className="section-subtitle">
Team captains should register first. Free agents can also sign up
individually and pay online to reserve their spot.
</p>
<div className="row">
<button className="primary" onClick={() => setPage("register")}>
Go to Registration
</button>
<button onClick={() => setPage("schedule")}>View Schedule Page</button>
<button onClick={() => setPage("waiver")}>Review Waiver</button>
</div>
</div>
</div>
</div>
...
onChange={(e) => setTeamForm((p) => ({ ...p, captainName: e.target.value }))}
placeholder="Captain / Coach Name"
/>
<input
value={teamForm.captainEmail}
onChange={(e) => setTeamForm((p) => ({ ...p, captainEmail: e.target.value }))}
placeholder="Captain Email"
/>
<input
value={teamForm.captainPhone}
onChange={(e) => setTeamForm((p) => ({ ...p, captainPhone: e.target.value }))}
placeholder="Captain Phone"
/>
<textarea readOnly value={fullWaiverText} rows={12} />
<input
value={teamForm.signature}
onChange={(e) => setTeamForm((p) => ({ ...p, signature: e.target.value }))}
...
onChange={(e) => setTeamForm((p) => ({ ...p, agreeWaiver: e.target.checked }))}
/>{" "}
...
onChange={(e) =>
setTeamForm((p) => ({ ...p, agreeCaptainDuty: e.target.checked }))
}
/>{" "}
I accept captain responsibility
</label>
<label>
<input
type="checkbox"
...
onChange={(e) =>
setTeamForm((p) => ({ ...p, agreeElectronic: e.target.checked }))
}
/>{" "}
I accept electronic signature
</label>
...
type="button"
className="primary"
disabled={savingTeam || startingCheckout}
...
onChange={(e) => setFreeAgentForm((p) => ({ ...p, playerName: e.target.value }))}
placeholder="Full Name"
/>
<input
value={freeAgentForm.playerEmail}
onChange={(e) => setFreeAgentForm((p) => ({ ...p, playerEmail: e.target.value }))}
placeholder="Email"
/>
<input
value={freeAgentForm.playerPhone}
onChange={(e) => setFreeAgentForm((p) => ({ ...p, playerPhone: e.target.value }))}
placeholder="Phone"
/>
<select
value={freeAgentForm.gender}
onChange={(e) => setFreeAgentForm((p) => ({ ...p, gender: e.target.value }))}
>
...
onChange={(e) => setFreeAgentForm((p) => ({ ...p, skill: e.target.value }))}
>
...
onChange={(e) =>
setFreeAgentForm((p) => ({ ...p, preferredPosition: e.target.value }))
}
...
onChange={(e) => setFreeAgentForm((p) => ({ ...p, signature: e.target.value }))}
placeholder="Type Full Legal Name as Signature"
/>
<label>
<input
...
onChange={(e) =>
setFreeAgentForm((p) => ({ ...p, agreeWaiver: e.target.checked }))
}
/>{" "}
I agree to the waiver
</label>
...
onChange={(e) =>
setFreeAgentForm((p) => ({ ...p, agreeElectronic: e.target.checked }))
}
/>{" "}
I accept electronic signature
</label>
...
type="submit"
className="primary"
disabled={savingFreeAgent || startingFreeAgentCheckout}
>
...
className="primary"
disabled={savingFreeAgent || startingFreeAgentCheckout}
onClick={handleFreeAgentCheckout}
>
{startingFreeAgentCheckout
? "Starting Checkout..."
: `Pay $${FREE_AGENT_PRICE} with Stripe`}
</button>
</div>
</form>
</div>
...
<p className="section-subtitle">
Weekly matchups will appear here after the admin generates the season schedule.
</p>
<div className="card">
...
<div className="card">
<div className="row">
<button className="primary" onClick={buildSchedule}>
Generate Round Robin
</button>
<button className="primary" onClick={assignPaidFreeAgentsToTeam}>
Assign Paid Free Agents
</button>
</div>
</div>
...
<td>
<StatusBadge status={getFreeAgentStatus(player)} />
</td>
</tr>
))}
</tbody>
